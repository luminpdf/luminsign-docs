...
### December 20, 2023
- Support signing order for Signature Request.
- Add `signing_type` parameter to [Send Signature Request](/api/send-signature-request) to specify the signing order. Possible values: `SAME_TIME` and `ORDER`.
- Add `group` parameter to [Send Signature Request](/api/send-signature-request) to specify the group of signers for the Signature Request with `signing_type` is `ORDER`.
