...
Lumin currently supports **Account-level webhooks** and **App-level webhooks**
    - **Account-level webhooks**: Configured once at the **Workspace level** (Workspace Owner sets it under **API Key → Account callback**). Covers all users and events in the Workspace, **independent of which API key or user initiated them**.
    - **App-level webhooks**: Configured once at the **App level** (App Owner sets it under **Developer settings → Integration apps → App webhook**). Events are limited to the **scopes** granted during OAuth authorization.
...
Lumin supports the following webhook events:
...
| `signature_request_due_date_updated` | The signature request due date is updated | [Signature Request](/api/get-signature-request/) |
...
- App webhook URLs subscribe to supported events limited to the **scopes** granted during OAuth authorization.
- Account webhook URLs subscribe to all supported events.

## Next steps

- See **[Account Webhooks](/docs/api/webhooks/account-callbacks)** for details on configuring Account-level callbacks.
- See **[App Webhooks](/docs/api/webhooks/app-callbacks)** for details on configuring App-level callbacks.