...
| :---------------- | :----- | :----------------------------------------------- |
| `response_type` | string | The value is `token` or `token id_token`                         |
...
| `nonce`        | string (required if request for `id_token`) | Value for prevent playback attack, you must compare with nonce in `id_token` if set  |
